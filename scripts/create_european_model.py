...
def _build_profit_and_loss(wb):
    ...
    operating_profit = [
        round(gross_profit[i] + distribution_costs[i] + admin_expenses[i] + other_operating_income[i], 1)
        for i in range(4)
    ]
    ...
    profit_before_tax = [
        round(operating_profit[i] + finance_costs[i] + finance_income[i], 1)
        for i in range(4)
    ]
    ...
    profit_for_year = [
        round(profit_before_tax[i] + taxation[i], 1)
        for i in range(4)
    ]
    # profit_for_year => [12.4, 14.8, 17.7, 20.8]

    _write_row(ws, r, "Turnover", turnover, bold=True);                       r += 1
    _write_row(ws, r, "Cost of Sales", cost_of_sales);                        r += 1
    _write_row(ws, r, "Gross Profit", gross_profit, bold=True);               r += 1
    r += 1  # blank row
    _write_row(ws, r, "Distribution Costs", distribution_costs);              r += 1
    _write_row(ws, r, "Administrative Expenses", admin_expenses);             r += 1
    _write_row(ws, r, "Other Operating Income", other_operating_income);      r += 1
    _write_row(ws, r, "Operating Profit", operating_profit, bold=True);       r += 1
    r += 1  # blank row
    _write_row(ws, r, "Finance Costs", finance_costs);                        r += 1
    _write_row(ws, r, "Finance Income", finance_income);                      r += 1
    _write_row(ws, r, "Profit Before Tax", profit_before_tax, bold=True);     r += 1
    _write_row(ws, r, "Taxation", taxation);                                  r += 1
    _write_row(ws, r, "Profit for the Year", profit_for_year, bold=True);     r += 1
...
def _build_balance_sheet(wb):
    ...
    # --- Non-Current Assets ---
    _write_section_header(ws, r, "Non-Current Assets");                        r += 1
    ...
    _write_row(ws, r, "  Property, Plant and Equipment", ppe);                r += 1
    _write_row(ws, r, "  Intangible Assets", intangibles);                    r += 1
    _write_row(ws, r, "  Goodwill", goodwill);                                r += 1
    _write_row(ws, r, "Total Non-Current Assets", total_nca, bold=True);      r += 1
    r += 1  # blank

    # --- Current Assets ---
    _write_section_header(ws, r, "Current Assets");                            r += 1
    stocks = [14.8, 15.5, 16.2, 17.0]
    trade_debtors = [24.8, 26.5, 28.5, 30.4]
    other_debtors = [3.2, 3.4, 3.6, 3.8]
    cash_at_bank = [18.5, 22.1, 28.0, 36.8]
    total_ca = [round(stocks[i] + trade_debtors[i] + other_debtors[i] + cash_at_bank[i], 1) for i in range(4)]
    # total_ca => [61.3, 67.5, 76.3, 88.0]

    _write_row(ws, r, "  Stocks", stocks);                                    r += 1
    _write_row(ws, r, "  Trade Debtors", trade_debtors);                      r += 1
    _write_row(ws, r, "  Other Debtors", other_debtors);                      r += 1
    _write_row(ws, r, "  Cash at Bank", cash_at_bank);                        r += 1
    _write_row(ws, r, "Total Current Assets", total_ca, bold=True);           r += 1
    r += 1  # blank

    total_assets = [round(total_nca[i] + total_ca[i], 1) for i in range(4)]
    # total_assets => [147.0, 154.1, 163.9, 176.5]
    _write_row(ws, r, "Total Assets", total_assets, bold=True);               r += 1
    r += 1  # blank

    # --- Current Liabilities ---
    _write_section_header(ws, r, "Current Liabilities");                       r += 1
    ...
    _write_row(ws, r, "  Trade Creditors", trade_creditors);                  r += 1
    _write_row(ws, r, "  Accruals", accruals);                                r += 1
    _write_row(ws, r, "  Provisions", provisions);                            r += 1
    _write_row(ws, r, "  Current Portion of Borrowings", current_borrowings); r += 1
    _write_row(ws, r, "Total Current Liabilities", total_cl, bold=True);      r += 1
    r += 1  # blank

    # --- Non-Current Liabilities ---
    _write_section_header(ws, r, "Non-Current Liabilities");                   r += 1
    ...
    _write_row(ws, r, "  Bank Borrowings", bank_borrowings);                  r += 1
    _write_row(ws, r, "  Other Non-Current Liabilities", other_ncl);          r += 1
    _write_row(ws, r, "Total Non-Current Liabilities", total_ncl, bold=True); r += 1
    r += 1  # blank

    total_liabilities = [round(total_cl[i] + total_ncl[i], 1) for i in range(4)]
    # total_liabilities => [75.7, 72.2, 68.8, 65.6]
    _write_row(ws, r, "Total Liabilities", total_liabilities, bold=True);     r += 1
    r += 1  # blank

    # --- Shareholders' Funds ---
    _write_section_header(ws, r, "Shareholders' Funds");                       r += 1
    ...
    _write_row(ws, r, "  Share Capital", share_capital);                      r += 1
    _write_row(ws, r, "  Retained Profit", retained_profit);                  r += 1
    _write_row(ws, r, "Total Shareholders' Funds", total_equity, bold=True);  r += 1
    r += 1  # blank
...
def _build_cash_flow(wb):
    ...
    _write_row(ws, r, "Cash Generated from Operations", cash_gen_ops, bold=True); r += 1
    _write_row(ws, r, "Taxation Paid", taxation_paid);                             r += 1
    _write_row(ws, r, "Net Cash from Operating Activities", net_operating, bold=True); r += 1
    r += 1  # blank
    ...
    _write_row(ws, r, "Purchase of Property, Plant and Equipment", purchase_ppe); r += 1
    _write_row(ws, r, "Purchase of Intangible Assets", purchase_intangibles);     r += 1
    _write_row(ws, r, "Net Cash Used in Investing Activities", net_investing, bold=True); r += 1
    r += 1  # blank
    ...
    _write_row(ws, r, "Repayment of Borrowings", repayment);                      r += 1
    _write_row(ws, r, "Dividends Paid", dividends_paid);                           r += 1
    _write_row(ws, r, "Net Cash Used in Financing Activities", net_financing, bold=True); r += 1
    r += 1  # blank

    net_increase = [round(net_operating[i] + net_investing[i] + net_financing[i], 1) for i in range(4)]
    ...
    _write_row(ws, r, "Net Increase / (Decrease) in Cash", net_increase, bold=True); r += 1
    _write_row(ws, r, "Cash at Beginning of Year", beg_cash);                        r += 1
    _write_row(ws, r, "Cash at End of Year", end_cash, bold=True);                   r += 1
...
def _build_debt_summary(wb):
    ...
    _write_row(ws, r, "Revolving Credit Facility", rcf);                      r += 1
    _write_row(ws, r, "Term Loan", term_loan);                                r += 1
    _write_row(ws, r, "Total Borrowings", total_borrowings, bold=True);       r += 1
    r += 1  # blank
    _write_row(ws, r, "Interest Rate", interest_rate, fmt=PCT_FMT);           r += 1
    _write_row(ws, r, "Interest Paid", interest_paid);                        r += 1
...
def _build_assumptions(wb):
    ...
    _write_row(ws, r, "Revenue Growth", revenue_growth, fmt=PCT_FMT);         r += 1
    _write_row(ws, r, "Gross Margin", gross_margin, fmt=PCT_FMT);             r += 1
    _write_row(ws, r, "Tax Rate", tax_rate, fmt=PCT_FMT);                     r += 1
    _write_row(ws, r, "Capex % Revenue", capex_pct, fmt=PCT_FMT);            r += 1
    _write_row(ws, r, "Dividend Payout Ratio", dividend_payout, fmt=PCT_FMT); r += 1
