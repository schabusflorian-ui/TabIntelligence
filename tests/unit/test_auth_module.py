...
class TestVerifyApiKey:
    ...
    @pytest.mark.asyncio
    async def test_returns_api_key_on_valid_key(self):
        ...
        result = await verify_api_key("emi_test123", mock_db)
        assert result == mock_api_key
        mock_db.execute.assert_called_once()

    @pytest.mark.asyncio
    async def test_returns_none_on_invalid_key(self):
        ...
        result = await verify_api_key("emi_invalid_key", mock_db)
        assert result is None

    @pytest.mark.asyncio
    async def test_hashes_key_before_lookup(self):
        ...
        await verify_api_key("emi_test_key_123", mock_db)

        # Verify execute was called (it uses the hashed key in the query)
        mock_db.execute.assert_called_once()
...
class TestCreateApiKey:
    ...
    @pytest.mark.asyncio
    async def test_creates_key_and_returns_record(self):
    ...
    @pytest.mark.asyncio
    async def test_creates_key_with_entity_id(self):
        ...
        api_key_record, _ = await create_api_key(
            mock_db, name="Entity Key", entity_id=entity_id
        )

        # Check that the key was added with entity_id
        added_obj = mock_db.add.call_args[0][0]
        assert added_obj.entity_id == entity_id

    @pytest.mark.asyncio
    async def test_creates_key_with_custom_rate_limit(self):
...
class TestRevokeApiKey:
    ...
    async def test_revokes_existing_key(self):
        ...
        result = await revoke_api_key(mock_db, uuid4())
...
class TestGetCurrentApiKey:
    ...
    @pytest.mark.asyncio
    async def test_returns_api_key_on_valid_credentials(self):
    ...
    @pytest.mark.asyncio
    async def test_raises_401_on_invalid_credentials(self):
...
class TestGetOptionalApiKey:
    ...
    async def test_returns_none_when_no_credentials(self):
        ...
        result = await get_optional_api_key(credentials=None, db=mock_db)
        assert result is None
    ...
    @pytest.mark.asyncio
    async def test_returns_none_when_credentials_invalid(self):
